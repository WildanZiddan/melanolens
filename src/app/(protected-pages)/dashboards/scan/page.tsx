...
// 🌐 URL Endpoint Real AI Backend FastAPI (Gated MobileNetV2 Model)
...
export default function AdminScanPage() {
    ...
    const handleUploadAndPredict = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Gagal terhubung dengan server AI MobileNetV2 FastAPI')
            }
            ...
            toast.push(
                <Notification title="Analisis MobileNetV2 Selesai" type="success">
    ...
    return (
        <div className="p-4 md:p-8 max-w-5xl mx-auto min-h-screen">
            ...
            {/* Judul Halaman Scan */}
            <div className="mb-6">
                <h3 className="font-bold mb-1 heading-text">Scan Foto Dermoskopi AI (MobileNetV2)</h3>
                <p className="text-slate-400 text-sm">Unggah foto lesi kulit Anda untuk analisis deteksi dini kanker Melanoma berbasis model Deep Learning MobileNetV2 dengan pengawasan lokalisasi lesi (gating).</p>
            </div>
                        ...
                        {selectedFile && (
                            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                ...
                                <Button
                                    ...
                                    disabled={isLoading}
                                >
                                    {isLoading ? 'Menganalisis dengan Model MobileNetV2...' : 'Mulai Analisis AI Real'}
                                </Button>
                            </div>
                        )}
                    </Card>
                    ...
                    <Card className="h-full flex flex-col">
                        ...
                        {!isLoading && !result && (
                            <div className="flex flex-col items-center justify-center flex-1 py-12 text-center bg-slate-50 dark:bg-slate-800/20 rounded-2xl border border-dashed border-slate-100 dark:border-slate-800 p-4">
                                ...
                                <p className="text-[11px] text-slate-400 leading-relaxed">Silakan unggah foto bercak kulit Anda untuk memicu analisis mesin Deep Learning MobileNetV2.</p>
                            </div>
                        )}
                                            ...
                                            <>
                                                ...
                                                {result.heatmap_base64 && (
                                                    <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800">
                                                        ...
                                                        <p className="text-[10px] text-slate-400 mt-1.5 leading-snug">Heatmap menyoroti fokus area jaringan lesi kulit yang dianalisis oleh model MobileNetV2.</p>
                                                    </div>
                                                )}
