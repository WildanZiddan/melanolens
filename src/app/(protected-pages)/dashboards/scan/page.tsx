...
import { TbCloudUpload, TbAlertTriangle, TbActivity, TbFileCheck, TbArrowLeft, TbUser } from 'react-icons/tb'
...
export default function AdminScanPage() {
    ...
    // State untuk data login admin
    const [adminId, setAdminId] = useState<number | string | null>(null)

    // Decode token dari localStorage untuk mendapatkan admin user_id
    useEffect(() => {
    ...
    // Saringan ukuran file maksimal 5 MB
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > maxSizeBytes) {
            toast.push(
                <Notification title="File Kebesaran!" type="danger">
                    Batas maksimal file gambar adalah 5 MB. Silakan kompres foto dulu!
                </Notification>
            )
    ...
    const handleUploadAndPredict = async () => {
        ...
        // 🔑 KUNCI SAKTI 1: Ambil ID murni dari state admin
        ...
        try {
            ...
            toast.push(
                <Notification title="Sukses Simpan Hasil" type="success">
            ...
            console.error(error)
            toast.push(
                <Notification title="Gagal Analisis" type="danger">
                    Terjadi kesalahan koneksi ke model AI.
                </Notification>
            )
        } finally {
            setIsLoading(false)
    ...
    return (
        <div className="p-4 md:p-6 w-full min-h-screen">
            {/* Judul Halaman Scan */}
            <div className="mb-6">
                <h3 className="font-bold mb-1 heading-text">Scan Foto Dermoskopi AI (Panel Admin)</h3>
                <p className="text-slate-400 text-sm">Unggah foto lesi kulit pasien Anda untuk melakukan diagnosis klinis awal Melanoma dengan AI.</p>
            </div>
                    ...
                    <Card className="h-full flex flex-col">
                        ...
                        {!isLoading && !result && (
                            <div className="flex flex-col items-center justify-center flex-1 py-12 text-center bg-slate-50 dark:bg-slate-800/20 rounded-2xl border border-dashed border-slate-100 dark:border-slate-800 p-4">
                                ...
                                <p className="text-[11px] text-slate-400 leading-relaxed">Silakan unggah foto bercak kulit pasien Anda terlebih dahulu untuk memicu mesin deteksi klinis.</p>
                            </div>
                        )}
